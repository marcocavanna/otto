...
Questa skill lavora **a valle** di `task-implementer`. Presuppone che esista il brief `docs/tasks/<id>.md` e, nella sua **context-root**, i file di contesto:
- `<context-root>/00-context.md`
- `<context-root>/02-abstract.md`
- `<context-root>/technical-context.md`

**Risoluzione della context-root**: leggere l'header del brief. `Context-root: docs/planning/` (project-planner) oppure `docs/features/<slug>/` (feature-planner). Se l'header manca → default `docs/planning/` (retro-compatibilità). Contratto canonico: `../feature-planner/feature-artifacts.md` § "Planning source contract". Da qui in poi, ogni riferimento ai file di contesto è relativo alla context-root risolta.

Se il brief o i file di contesto non esistono, la skill rifiuta di operare e indirizza alle skill upstream.
...
2. **Mimic prima di innovare.** Se esiste già un costrutto simile nel codebase (un altro controller, un altro repository, ecc.), il nuovo codice ne segue lo stile. Solo per il **primo** costrutto di una categoria, la skill applica i template di `technical-context.md` direttamente.
...
- **Read-only**: `docs/planning/*`, `docs/tasks/T-NNN.md`, codice esistente del progetto
- **Write**: codice sorgente del progetto, sezioni specifiche di `docs/tasks/T-NNN.md`, `technical-context.md` (solo per decisioni cross-task confermate)

Regola di scope: la skill **non** modifica `02-abstract.md`, `00-context.md`, `03-milestones.md`, `04-phases.md`, `05-tasks-active.md`. Se qualcuno di questi va modificato, indirizza a `project-planner` (revise).
...
1. **Pre-flight check** (vedi `references/preflight.md`):
   ...
   - **Risolve la context-root** dall'header `Context-root:` del brief (default `docs/planning/`)
   - Esistenza dei file di contesto nella context-root risolta
   - Build command è dichiarato nel `technical-context.md` della context-root (se no: warning)

2. **Context loading** (vedi `references/context-loading.md`):
   - Legge i 3 file di contesto **dalla context-root risolta** + il brief
   ...
   - Se non trovato: prima volta → segue solo i template di `technical-context.md`
